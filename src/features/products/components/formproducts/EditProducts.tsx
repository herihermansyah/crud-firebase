"use client";
import React, {ChangeEvent, FormEvent, useEffect, useState} from "react";
import {Products, ProductsWithId} from "../../types/products.types";
import {useParams, useRouter} from "next/navigation";
import {doc, getDoc, updateDoc} from "firebase/firestore";
import {db} from "@/lib/firebaseConfig";
import {field, selectField, sizes} from "../../constants/fieldProducts";
import {
  Button,
  Checkbox,
  FormControl,
  InputLabel,
  MenuItem,
  Select,
  SelectChangeEvent,
  TextField,
} from "@mui/material";
import {initialProducts} from "../../constants/initialProducts";
import ButtonBack from "@/components/ui/ButtonBack";
import {motion} from "motion/react";
import {toast} from "react-toastify";

function EditProducts() {
  const [productID, setProductID] = useState<ProductsWithId>(() => ({
    ...initialProducts,
    id: "",
  }));
  const {id} = useParams();
  const router = useRouter();
  useEffect(() => {
    const fetchDataId = async () => {
      try {
        const docRef = doc(db, "products", id as string);
        const docSnap = await getDoc(docRef);
        if (docSnap.exists()) {
          setProductID({
            id: docSnap.id,
            ...(docSnap.data() as Omit<Products, "id">),
          });
        }
      } catch {
        return (
          <div className="flex justify-center items-center min-h-screen">
            <span>gagal fetching data</span>
          </div>
        );
      }
    };
    if (id) fetchDataId();
  }, [id]);

  const handleChange = (e: ChangeEvent<HTMLInputElement>) => {
    const {name, value} = e.target;
    setProductID((prev) => ({
      ...prev,
      [name]: value,
    }));
  };

  const handleSelect = (e: SelectChangeEvent<string>) => {
    const {name, value} = e.target;
    setProductID((prev) => ({
      ...prev,
      [name]: value,
    }));
  };

  const handleEditProduct = async (e: FormEvent) => {
    e.preventDefault();
    try {
      if (!productID) return;
      const {id: productDocID, ...updateProduct} = productID;
      await updateDoc(doc(db, "products", productDocID), updateProduct);
      toast.success("berhasil edit product");
      router.push("/products");
    } catch {
      toast.error("gagal edit product");
    }
  };
  return (
    <div className="container mx-auto px-2 flex flex-col gap-10 ">
      <ButtonBack />
      <motion.div
        initial={{opacity: 0, scale: 0}}
        animate={{opacity: 1, scale: 1}}
        transition={{duration: 0.2}}
        className="border border-gray-300 p-5 rounded-2xl shadow-2xl"
      >
        <form className="flex flex-col gap-4" onSubmit={handleEditProduct}>
          {field.map((item, index) => (
            <TextField
              key={index}
              label={item.label}
              name={item.key}
              type={item.type}
              value={productID[item.key] || ""}
              onChange={handleChange}
              required
            />
          ))}

          <TextField
            name="discount"
            label="discount"
            type="number"
            value={productID.discount}
            onChange={handleChange}
          />

          <TextField
            name="thumbnail"
            label="thumbnail"
            type="text"
            value={productID.thumbnail}
            onChange={handleChange}
          />
          <div className="border border-gray-300 rounded-md p-5">
            <h3 className="mb-5 capitalize">size</h3>
            <div className="border flex flex-wrap items-center border-gray-300">
              {sizes.map((sz) => {
                const existing = productID.size.find((s) => s.size === sz);

                return (
                  <div key={sz} className="p-4">
                    <div className="flex items-center">
                      <Checkbox
                        checked={!!existing}
                        onChange={(e) => {
                          if (e.target.checked) {
                            setProductID((prev) => ({
                              ...prev,
                              size: [...prev.size, {size: sz, stock: 0}],
                            }));
                          } else {
                            setProductID((prev) => ({
                              ...prev,
                              size: prev.size.filter((s) => s.size !== sz),
                            }));
                          }
                        }}
                      />
                      <span>{sz.toUpperCase()}</span>
                    </div>

                    <div>
                      <TextField
                        type="number"
                        label="stock"
                        size="small"
                        required
                        disabled={!existing}
                        value={existing?.stock ?? ""}
                        onChange={(e) => {
                          const newStock = Number(e.target.value);
                          setProductID((prev) => ({
                            ...prev,
                            size: prev.size.map((item) =>
                              item.size === sz
                                ? {...item, stock: newStock}
                                : item
                            ),
                          }));
                        }}
                      />
                    </div>
                  </div>
                );
              })}
            </div>
          </div>
          {selectField.map((item, index) => (
            <FormControl key={index}>
              <InputLabel>{item.key}</InputLabel>
              <Select<string>
                name={item.key}
                label={item.label}
                required
                value={productID[item.key] as string}
                onChange={handleSelect}
              >
                {item.value?.map((item, index) => (
                  <MenuItem key={index} value={item}>
                    {item}
                  </MenuItem>
                ))}
              </Select>
            </FormControl>
          ))}
          <Button type="submit" variant="contained">
            add
          </Button>
        </form>
      </motion.div>
    </div>
  );
}

export default EditProducts;
